...
class ScriptTaskExecutor(TaskExecutor):
    ...
    async def _execute_python_script(self, script: str) -> Dict[str, Any]:
        """Execute a Python script step via a subprocess for isolation.

        The previous implementation used exec() with a bare-builtins dict which
        still allowed access to dunder attributes and was not truly sandboxed.
        We now run the script as a child process so the parent process is not
        exposed to arbitrary code paths.
        """
        import subprocess, sys, json as _json, tempfile, os as _os

        # Write script to a temporary file so we don't pass arbitrary code via
        # a shell string (avoids shell-injection issues too).
        try:
            ...
            return {
                "output": result.stdout or result.stderr or "Script executed",
                "exit_code": result.returncode,
                "variables": {},
            }
        except subprocess.TimeoutExpired:
            return {"output": "Script timed out after 30s", "exit_code": 124}
        except Exception as e:
            return {"output": f"Script execution error: {e}", "exit_code": 1, "error": str(e)}
        finally:
            try:
                _os.unlink(tmp_path)
            except Exception:
