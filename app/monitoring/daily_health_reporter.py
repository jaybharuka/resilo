...
class HealthReportGenerator:
    ...
    def send_slack_report(self, report: DailyHealthReport, webhook_url: str = None):
        """Send a beautifully formatted daily summary to Slack"""
        import os
        import requests
        ...
        # Format the risks and recommendations
        risk_text = "\n".join([f"• *{r.primary_cause}*: {r.duration_minutes}m ({r.risk_level.upper()})" for r in report.peak_risk_periods])
        if not risk_text:
            risk_text = "None! 🎉"
            
        recs_text = "\n".join([f"• {rec}" for rec in report.recommendations])
        
        color = "#36A64F" if report.overall_score >= 80 else ("#FFCC00" if report.overall_score >= 60 else "#FF0000")
            ...
            "blocks": [
                {
                    ...
                    "text": {
                        "type": "plain_text",
                        "text": f"📊 Daily AIOps Health Report: {report.date}"
                    }
                },
                {
                    "type": "section",
                    "fields": [
                        {
                            "type": "mrkdwn",
                            "text": f"*Overall Health Score:*\n{report.overall_score}/100"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Avg CPU Load:*\n{report.cpu_analysis.get('average', 0):.1f}% (Peak {report.cpu_analysis.get('peak', 0):.1f}%)"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Avg Memory Used:*\n{report.memory_analysis.get('average', 0):.1f}% (Peak {report.memory_analysis.get('peak', 0):.1f}%)"
                        },
                        {
                            "type": "mrkdwn",
                            "text": f"*Risk Periods Detected:*\n{len(report.peak_risk_periods)}"
                        }
                    ]
                }
            ],
