...
def _check_pw(stored_hash: str, plain_password: str) -> bool:
    """Verify a password against a stored hash.

    Handles both legacy bcrypt hashes (created by auth_system.py / bcrypt library,
    prefixed with $2b$, $2a$, $2y$) and Werkzeug scrypt/pbkdf2 hashes so that users
    created by either backend can log in.
    """
    if not stored_hash:
        return False
...
def auth_login():
    ...
    try:
        ...
        row = cur.fetchone()
        if not row or not _check_pw(row['password_hash'], password):
            return jsonify({'error': 'invalid_credentials'}), 401
        # Migrate legacy bcrypt hashes to Werkzeug scrypt on first successful login
        if row['password_hash'].startswith(('$2b$', '$2a$', '$2y$')):
            try:
                new_hash = generate_password_hash(password)
                with conn:
                    conn.execute('UPDATE users SET password_hash = ? WHERE id = ?', (new_hash, row['id']))
            except Exception:
                pass  # Migration is best-effort; login still succeeds
